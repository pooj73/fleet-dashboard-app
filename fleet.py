...
# 1️⃣ Load your 50 row Excel sheet
...
# Convert to M (million) / K (thousand)
...
def generate_ai_report():
    ...
    top_routes = "A & C"  # Example, modify as per your logic for route analysis
    avg_profit_per_trip = round(net_profit / total_trips, 2)

    ai_report = f"""
    📊 **AI Report Highlights:**

    **Total Trips:** {total_trips}
    **On-going Trips:** {ongoing_trips}
    **Completed Trips:** {closed_trips}
...
<body class="bg-[#0B132B] text-white font-sans p-6">
  ...
  <div class="grid grid-cols-4 gap-4 mb-6">
    ...
    <div class="bg-[#1C2541] p-4 rounded col-span-2">
      ...
      <div class="grid grid-cols-3 gap-4 text-sm mb-4">
        <div>
          <p class="text-xl font-bold">₹{{ rev_m }} M</p>
          <p class="text-gray-400">Revenue</p>
        </div>
        <div>
          <p class="text-xl font-bold">₹{{ profit_m }} M</p>
          <p class="text-gray-400">Profit</p>
        </div>
        <div>
          <p class="text-xl font-bold">{{ kms_k }} K</p>
          <p class="text-gray-400">KM Travelled</p>
        </div>
      </div>
      ...
      <div class="flex items-center justify-between text-sm">
        ...
        <button class="bg-green-600 px-4 py-1 rounded font-semibold hover:bg-green-700">
          ₹{{ per_km }}
        </button>
      </div>
    </div>
  ...
  <div class="grid grid-cols-3 gap-4">
    ...
    <div class="bg-[#1C2541] p-4 rounded col-span-2">
      ...
      <a href="/download-summary" class="mt-4 inline-block bg-blue-600 px-4 py-2 rounded text-white hover:bg-blue-700">
        Download Summary
      </a>
    </div>
  </div>
...
def download_summary():
    ...
    # Save the AI report to a file with utf-8 encoding to avoid UnicodeEncodeError
    summary_file = "ai_report_summary.txt"
